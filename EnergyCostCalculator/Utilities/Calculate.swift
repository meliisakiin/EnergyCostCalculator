...
class CostCalculationServiceDelegate : CostCalculationService {
    ...
    private final func validateSerialNumber(serialNumber: String) -> (isValid: Bool,errorFields: (fieldName: String, inputValue : String)?){
        do {
            ...
            if(value.count < 10){
                self.delegate.handleSerialNumber(value: Messages.serialNumber.localized())
                return (false, (fieldName: "SerialNumber", inputValue: Messages.serialNumber.localized()))
            }else{
    ...
    private final func filterArray(serialNumber: String)-> [HistoryModel]{
        let filtered = self.history.filter { $0.serialNumber == serialNumber }
    ...
    private final func validateCurrentReading(currentReading: String) -> (isValid : Bool,errorFields: (fieldName: String, inputValue : String)?){
        do {
            ...
            if(reading.isInt && (Int(reading) ?? 0) > (lastReading?.units ?? 0)){
                ...
                self.delegate.handleCurrentReading(value: Messages.currentReading.localized())
                return (false, (fieldName: "CurrentReading", inputValue: Messages.currentReading.localized()))
            }
        ...
        case .failure(_, let fields):
            ...
            for field in fields {
                if(field.contains("CurrentReading")){
                    fieldsArr.append(HandleUIErrorModel(key: "CurrentReading", value: Messages.emptyCurrentReading.localized()))
                }
                if(field.contains("SerialNumber")){
                    fieldsArr.append(HandleUIErrorModel(key: "SerialNumber", value: Messages.emptySerialNumber.localized()))
                }
            }
